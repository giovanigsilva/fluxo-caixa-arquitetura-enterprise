...
"""),
        new(
            "agent.modes",
            "Agente Vertx",
            ["agente", "robo", "chat", "conversar", "ligacao", "telefone", "local"],
            """
O Agente Vertx fica fixo no canto inferior direito da tela autenticada. Ao clicar no ícone, aparecem três opções acima dele: Conversar por chat, Conversar local e Conversar por ligação. Conversar por chat chama o SupportAgent.Api, que usa RAG governado e LLM Qwen local em GPU. Conversa local e ligação estão preparados visualmente; microfone, discagem real e provedor telefônico serão definidos depois.
"""),
        new(
            "agent.rag_policy",
